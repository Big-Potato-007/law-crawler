...
from .models import CrawlResult, SearchConfig, classify_doc_type_and_category, infer_issuer
...
class WebCrawler:
    ...
    def search(self, config: SearchConfig) -> List[str]:
        ...
        for page in range(1, config.max_pages + 1):
            candidate_urls = (
                source_adapter.build_search_urls(config.keyword, page)
                if hasattr(source_adapter, "build_search_urls")
                else [source_adapter.build_search_url(config.keyword, page)]
            )
    ...
    def crawl_urls(
        ...
        for idx, url in enumerate(urls):
            ...
            doc_type, _ = classify_doc_type_and_category(title, soup.get_text(" ", strip=True))
            issuer = self._extract_issuer(soup, title)
    ...
    @staticmethod
    def _extract_issuer(soup: BeautifulSoup, title: str) -> Optional[str]:
